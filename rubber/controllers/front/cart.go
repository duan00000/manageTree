...
func (c *CartController) Get() {
	...
	c.TplName = "front/cart/cart.html"
}
...
func (c *CartController) AddCart() {
	...
	*/
	...
	errInfoGId := models.DB.Where("id=?", goodsId).Find(&goods).Error
	errInfoCId := models.DB.Where("id=?", colorId).Find(&goodsColor).Error

	if errCId != nil || errGId != nil || errInfoGId != nil || errInfoCId != nil {
		c.Ctx.Redirect(302, "/item_"+strconv.Itoa(goods.Id)+".html")
		return
	}
	//	1、获取增加购物车的数据 （加入购物车的商品）
	currentData := models.Cart{
		...
		GoodsFitting: goods.GoodsFitting,
	...
	c.Data["goods"] = goods
	c.SuperInit()
	c.TplName = "front/cart/addCart_success.html"
}
