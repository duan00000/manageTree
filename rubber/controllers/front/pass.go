...
func (c *PassController) Login() {
	...
	c.TplName = "front/pass/login.html"
}
...
func (c *PassController) DoLogin() {
	...
	photoCode := c.GetString("photo_code")
	photoCodeId := c.GetString("photoCodeId")

	// 1、验证图形验证码是否正确
	identifyFlag := models.Cpt.Verify(photoCodeId, photoCode)
	if !identifyFlag {
	...
	user := []models.User{}
	models.DB.Where("phone=? AND password=? AND status=?", phone, password, 0).Find(&user)
	if len(user) > 0 { //登录成功
...
func (c *PassController) RegisterStep1() {
	...
	c.TplName = "front/pass/register_step1.html"
}

func (c *PassController) RegisterStep2() {
	var sign = c.GetString("sign")
	var photoCode = c.GetString("photo_code")

	//1、图形验证码是否正确
	sessionPhotoCode := c.GetSession("photo_code")
	if photoCode != sessionPhotoCode {
	...
	if len(userTemp) > 0 {
		c.Data["sign"] = sign
		c.Data["photo_code"] = photoCode
		c.Data["phone"] = userTemp[0].Phone
		c.TplName = "front/pass/register_step2.html"
	} else {
		c.Redirect("/pass/registerStep1", 302)
		return
	}
...
func (c *PassController) RegisterStep3() {
	...
	if len(userTemp) > 0 {
		...
		c.TplName = "front/pass/register_step3.html"
	} else {
		c.Redirect("/pass/registerStep1", 302)
		return
	}
...
func (c *PassController) SendCode() {
	...
	if photoCodeId == "resend" { //重新发送验证码需要用到
		//session里面验证验证码是否合法 与url传入验证比较
		sessionPhotoCode := c.GetSession("photo_code")
		if sessionPhotoCode != photoCode {
			c.Data["json"] = map[string]interface{}{
				"success": false,
		...
		//1、验证图形验证码是否正确
		if !models.Cpt.Verify(photoCodeId, photoCode) {
			c.Data["json"] = map[string]interface{}{
				"success": false,
				"msg":     "输入的图形验证码不正确",
			}
		...
		c.SetSession("photo_code", photoCode)
	}
	...
	*/
	...
	smsCode := models.GetRandomNum()                      //发送短信的随机码

	userTemp := []models.UserTemp{}
	models.DB.Where("add_day=? AND phone=?", addDay, phone).Find(&userTemp)
	// 隔天 刷新SendCount 记录
	if len(userTemp) > 0 {
		userTempNew := models.UserTemp{}
...
//用户名
func (c *PassController) ValidateUsername() {
	username := c.GetString("username")
	//用户名是否已注册
	usernameExit := []models.User{}
	models.DB.Where("username=?", username).Find(&usernameExit)
