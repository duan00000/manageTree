...
func (c *BuyController) Checkout() {
	...
	//3、防止重复提交订单 生成签名
	orderSign := models.Md5(models.GetRandomNumSix())
	c.SetSession("orderSign", orderSign)
	c.Data["orderSign"] = orderSign

	c.TplName = "front/buy/checkout.html"
}
...
func (c *BuyController) DoOrder() {
	...
	if len(addressResult) > 0 {
		...
		//  3、把订单信息放在订单表，把商品信息放在商品表  属性：models.order.go
		order := models.Order{
...
func (c *BuyController) Confirm() {
	...
	c.TplName = "front/buy/confirm.html"
}
